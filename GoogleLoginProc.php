...
include "dbconn.php";
$email = $_POST["email"];
$regist_result = mysqli_query($conn, "SELECT * FROM user where sns_email = '$email'");
...
if(!$regist_row){
	...
	mysqli_query($conn,"INSERT INTO user (sns_email, password, name, phone, birth, gender, findpw_q, findpw_a, status, img, sns_login, created_at) VALUES ('$email', '$id', '$name', '$phone', '$birth', '$gender', '$findpw_q', '$findpw_a', '$status', '$img', '1', '$created_at')");
	sleep(2);
	if($status == 1){
