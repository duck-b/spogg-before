...
//타자
$hit_record_query = "SELECT test_record_hit.*, YEAR(game_date) as game_years FROM test_record_hit WHERE player='$player_no' ORDER BY game_date DESC, created_at";
...
while($hit_record_row = mysqli_fetch_array($hit_record_result)){
    $i++;
    if($now_years != $hit_record_row['game_years']){
        $now_years = $hit_record_row['game_years'];
        $years_count++;
    }
    ...
    if($hit_record_row['kind'] == 1){
        $kind = '연습경기';
    }else{
        $kind = '용병경기';
    }
    $hit_table_basic = $hit_table_basic."<tr>
        <td>".$i."</td><td>".$kind."</td><td>".$hit_record_row['game_date']."</td><td>".$hit_record_row['team_name']."</td><td>".$avg."</td><td>".$hit_record_row['at_play']."</td><td>".$hit_record_row['at_bat']."</td><td>".$hit_record_row['hit']."</td><td>".$hit2."</td><td>".$hit3."</td><td>".$hit_record_row['hr']."</td><td>".$hit_record_row['rbi']."</td><td>".$rs."</td><td>".$sb."</td><td>".$hit_record_row['bb']."</td><td>".$hbp_text."</td><td>".$hit_record_row['so']."</td><td>".$sf."</td><td>".$tb."</td><td>".$xbh."</td><td>".$obp."</td><td>".$slg."</td><td>".$ops."</td><td><a href='javascript:;' onclick='onclick=updaterecodeform(2,$hit_no)'><i class='fa fa-edit' style='color:green'></i></a></td><td><a href='javascript:;' onclick='delete_event($hit_no,1)'><i class='fa fa-trash' style='color:red'></i></a></td>
    </tr>";
}
if($i > 0){
    $hit_summary_query = "SELECT user_record.*, league_record.years_avg FROM 
                            (SELECT YEAR(game_date) AS league_years,
                                COUNT(*) AS at_game,
                            ...
                            GROUP BY league_years) AS user_record 
                        JOIN 
                        ...
                        (SELECT 0 AS league_years,
                                ...
                                SUM(CASE WHEN sf != '9999' THEN sf END) AS sf,
                                COUNT(CASE WHEN sf != '9999' THEN 1 END) AS sf_count,
                                (MAX(YEAR(game_date)) - MIN(YEAR(game_date)) + 1) AS years_avg 
                        FROM test_record_hit  
    ...
    while($hit_summary_row = mysqli_fetch_array($hit_summary_result)){
        ...
        if($hit_summary_row['rs'] == null){
            ...
            if($hit_summary_row['rs_count'] == $hit_summary_row['at_game']){
        ...
        if($hit_summary_row['sb'] == null){
            ...
            if($hit_summary_row['sb_count'] == $hit_summary_row['at_game']){
        ...
        if($hit_summary_row['hit1'] == null ||$hit_summary_row['hit2'] == null || $hit_summary_row['hit3'] == null){
            ...
            if($hit_summary_row['hit1_count'] == $hit_summary_row['at_game']){
                if($hit_summary_row['at_bat'] != 0){
        ...
        if($hit_summary_row['league_years'] != 0){
            $hit_table_5years .= "<tr><td>".$hit_summary_row['league_years']."</td><td>".$at_game."</td><td>".$avg_text."</td><td>".$hit_summary_row['at_bat']."</td><td>".$hit_summary_row['hit']."</td><td>".$hit_summary_row['hr']."</td><td>".$hit_summary_row['bb']."</td><td>".$rs."</td><td>".$hit_summary_row['rbi']."</td><td>".$sb."</td><td>".$obp_text."</td><td>".$slg_text."</td><td>".$ops_text."</td></tr>";
            if($j > 4){
            ...
            $hit_table_summary = "<td>".number_format($hit_summary_row['years_avg'],0)."</td><td>".$at_game."</td><td>".$avg_text."</td><td>".$hit_summary_row['at_bat']."</td><td>".$hit_summary_row['hit']."</td><td>".$hit_summary_row['hr']."</td><td>".$hit_summary_row['bb']."</td><td>".$rs."</td><td>".$hit_summary_row['rbi']."</td><td>".$sb."</td><td>".$obp_text."</td><td>".$slg_text."</td><td>".$ops_text."</td>";
...
while($pit_record_row = mysqli_fetch_array($pit_record_result)){
    ...
    if($pit_record_row['kind'] == 1){
        $kind = '연습경기';
    }else{
        $kind = '용병경기';
    }
    if($pit_record_row['game_result'] == 1){
        $game_result = "승";
    }else if($pit_record_row['game_result'] == 2){
        $game_result = "패";
    }else if($pit_record_row['game_result'] == 3){
        $game_result = "홀";
    }else if($pit_record_row['game_result'] == 4){
        $game_result = "세";
    }else{
        $game_result = "-";
    }
    $pit_table_basic = $pit_table_basic."<tr>
        <td>".$i."</td><td>".$kind."</td><td>".$pit_record_row['game_date']."</td><td>".$pit_record_row['team_name']."</td><td>".$era."</td><td>".$rsa."</td><td>".$game_result."</td><td>".$pitcher_count."</td><td>".$h_inning.$f_inning."</td><td>".$er."</td><td>".$rs."</td><td>".$pit_record_row['hit']."</td><td>".$pit_record_row['hr']."</td><td>".$pit_record_row['bb']."</td><td>".$pit_record_row['hbp']."</td><td>".$pit_record_row['so']."</td><td>".$pit_record_row['whip']."</td><td><a href='javascript:;' onclick='onclick=updaterecodeform(3,$pit_no)'><i class='fa fa-edit' style='color:green'></i></a></td><td><a href='javascript:;' onclick='delete_event($pit_no,2)'><i class='fa fa-trash' style='color:red'></i></a></td>
    </tr>";
}
if($pit_table_basic){
    $pit_record_summary_query = "SELECT COUNT(CASE WHEN player='$player_no' THEN 1 END) AS years, 
            COUNT(CASE WHEN player='$player_no' THEN 1 END) AS at_game,
            COUNT(CASE WHEN game_result='1' THEN 1 END) AS win_game,
            COUNT(CASE WHEN game_result='2' THEN 1 END) AS lose_game,
        ...
        FROM test_record_pit WHERE player='$player_no'";
    ...
    if($pit_record_summary_row['win_game']+$pit_record_summary_row['lose_game'] != 0){
        $pct = number_format(round($pit_record_summary_row['win_game']/($pit_record_summary_row['win_game']+$pit_record_summary_row['lose_game'] != 0),3),3);
    }else{
        $pct = "-";
    }
    ...
    $pit_years_query = "SELECT COUNT(*) as pit_years FROM (SELECT YEAR(game_date) AS years FROM test_record_pit WHERE player='$player_no' GROUP BY YEAR(game_date))pit_count WHERE 1";
    $pit_years_result = mysqli_query($conn,$pit_years_query);
    $pit_years_row = mysqli_fetch_array($pit_years_result);
    $pit_table_summary = "<td>".$pit_years_row['pit_years']."</td><td>".$era." (".$pit_record_summary_row['er_c'].")</td><td>".$rsa." (".$pit_record_summary_row['rs_c'].")</td><td>".$pit_record_summary_row['at_game']."</td><td>".$pit_record_summary_row['win_game']."</td><td>".$pit_record_summary_row['lose_game']."</td><td>".$pct."</td><td>".$pit_record_summary_row['er']." (".$pit_record_summary_row['er_c'].")</td><td>".$pit_record_summary_row['rs']." (".$pit_record_summary_row['rs_c'].")</td><td>".$h_inning.$f_inning."</td><td>".$pit_record_summary_row['hit']."</td><td>".$pit_record_summary_row['so']."</td><td>".$pit_record_summary_row['whip']."</td>";
    mysqli_close($conn);
}
?>
<?if($item != 1){?>
	...
	<div class="container record_contents">
			...
			<div class="col-xs-12 col-md-3 text-right record_select_kinds">
				<form>
					<select class="rounded shadow" id="select_kind" onchange="change_kind()">
						...
						<option value="" selected>연습·용병기록</option>
					</select>
				</form>
			</div>
		</div>
        ...
        <div class="row">
            ...
            <div class="table-responsive to_mb_30 rounded shadow">
                <table class="table table_h_g record_table">
                    <tr>
                        <th>년차</th><th>경기</th><th>타율</th><th>타수</th><th>안타</th><th>홈런</th><th>볼넷</th><th>득점</th><th>타점</th><th>도루</th><th>출루율</th><th>장타율</th><th>OPS</th>
                    </tr>
                    <tr>
                        <?=$hit_table_summary;?>
                    </tr>
        ...
        <div class="row">
            ...
            <div class="table-responsive to_mb_30 rounded shadow">
                <table class="table table_h_g">
                    <tr>
                        <th>년도</th><th>경기</th><th>타율</th><th>타수</th><th>안타</th><th>홈런</th><th>볼넷</th><th>득점</th><th>타점</th><th>도루</th><th>출루율</th><th>장타율</th><th>OPS</th>
                    </tr>
                    <?=$hit_table_5years?>
                </table>
            </div>
		...
		<div class="table-box-wrap">
			<div class="table-responsive to_mb_30 table-box rounded shadow">
				<table class="table table_h_g recode_hitplay ro_table_12" id="recode_hitplay1">
					<thead>
						<tr>
							<th>No</th><th>경기구분</th><th>경기일</th><th>팀명</th><th>타율</th><th>타석</th><th>타수</th><th>안타</th><th>2루타</th><th>3루타</th><th>홈런</th><th>타점</th><th>득점</th><th>도루</th><th>볼넷</th><th>사사구</th><th>삼진</th><th>희생</th><th>루타</th><th>장타</th><th>출루율</th><th>장타율</th><th>OPS</th><th>수정</th><th>삭제</th>
						</tr>
					</thead>
					<tbody>
						<?=$hit_table_basic;?>
		...
		<div class="table-responsive to_mb_30 ">
			<table class="table table_h_g">
				<thead>
					<tr>
						<th>년차</th><th>평균자책점</th><th>실점율</th><th>경기</th><th>승</th><th>패</th><th>승률</th><th>자책</th><th>실점</th><th>이닝</th><th>피안타</th><th>탈삼진</th><th>출루허용율</th>
					</tr>
				</thead>
				<tbody>
					<tr>
		...
		<div class="table-box-wrap">
			<div class="table-responsive to_mb_30 table-box">
				<table class="table table_h_g recode_pitcherplay ro_table_12" id="recode_pitchplay">
					<thead>
						<tr>
							<th>No</th><th>경기구분</th><th>경기일</th><th>팀명</th><th>평균자책점</th><th>실점율</th><th>경기결과</th><th>투구</th><th>이닝</th><th>자책</th><th>실점</th><th>피안타</th><th>피홈런</th><th>볼넷</th><th>사사구</th><th>탈삼진</th><th>출루허용율</th><th>수정</th><th>삭제</th>
						</tr>
					</thead>
					<tbody>
						<?=$pit_table_basic;?>
...
function inputrecode() {
	...
	append_content ="<div class='to_titlebar'>연습·용병 기록</div><hr>" 
...
function inputrecodeform(check) {
	...
	var selectbox;
	var selectbox2 = "<option value=''>월</option>";
	...
	for(var m=1; m<13 ; m++){		
		selectbox2 += "<option value='"+m+"'>"+m+"</option>"; 
	}
	modal_body_inputrecode.empty();
	modal_footer_inputrecode.empty();
	if(check==0){				//타자 - 비공식 기록 입력
		append_content = "<div class='inputrecodeform' >"
						...
						+"	</div>"
						+"	<hr>"
						+"	<form action='TestRecordInsertProc.php' method='post'>"
						+"		<div class='form-group' id='divLeagueYear'>"
						+"	        <div class='col-xs-3 control-label'><label for='inputLeagueYear'>날짜<span style='color:red'> *</span></label></div>"
						+"	        <div class='col-xs-3 input_record_form'>"
						+"	            <select class='form-control' name='game_y' id='game_y'>"
						+				selectbox	
						+"	           	</select>" 
						+"	        </div>"
						+"	        <div class='col-xs-3 input_record_form'>"
						+"	            <select class='form-control' name='game_m' id='game_m' onchange='selectmonth(this.value)'>"
						+				selectbox2	
						+"	           	</select>" 
						+"	        </div>"
						+"	        <div class='col-xs-3 input_record_form'>"
						+"	            <select class='form-control' name='game_d' id='game_d'>"
						+"					<option value=''>일</option>"	
						+"	           	</select>" 
						+"	        </div>"
						+"	    </div>"
						+"	    <div class='form-group' id='divLeaguename'>"
						+"	        <div class='col-xs-3 control-label'><label for='inputLeaguename'>경기구분</label></div>"
						+"	        <div class='col-xs-9 input_record_form'>"
						+"	            <div class='col-xs-6 col-sm-6 col-md-6'>"
						+"	            	<input type='radio' id='kinds_1' name='kind' value='1' checked onclick='check_kind(1)'/><label for='kinds_1'>연습경기</label>"
						+"	            </div>"
						+"	            <div class='col-xs-6 col-sm-6 col-md-6'>"
						+"	            	<input type='radio' id='kinds_2' name='kind' value='2' onclick='check_kind(2)'/><label for='kinds_2'>용병경기</label>"
						+"	            </div>"
						+"	        </div>"
						+"	    </div>"
						...
						+"	        <div class='col-xs-9 input_record_form'>"
						+"	            <input type='text' class='form-control' name='team_name' id='team_name' maxlength='20'>"
						+"	        </div>"
						+"	    </div>"
		...
		append_content = "<div class='inputrecodeform' >"
						...
						+"	        </div>"
						+"	    </div>"
						+"	    <div class='form-group' id='divGameLesult'>"
						+"	        <div class='col-xs-3 control-label'><label for='divGameLesult'>경기결과</label></div>"
						+"	        <div class='col-xs-9 input_record_form'>"
						+"	            <div class='col-xs-1 col-sm-1 col-md-1'></div>"
						+"	            <div class='col-xs-2 col-sm-2 col-md-2'>"
						+"	            	<input type='radio' id='game_result_0' name='game_result' value='0' checked/><label for='game_result_0'>-</label>"
						+"	            </div>"
						+"	            <div class='col-xs-2 col-sm-2 col-md-2'>"
						+"	            	<input type='radio' id='game_result_1' name='game_result' value='1'/><label for='game_result_1'>승</label>"
						+"	            </div>"
						+"	            <div class='col-xs-2 col-sm-2 col-md-2'>"
						+"	            	<input type='radio' id='game_result_2' name='game_result' value='2'/><label for='game_result_2'>패</label>"
						+"	            </div>"
						+"	            <div class='col-xs-2 col-sm-2 col-md-2'>"
						+"	            	<input type='radio' id='game_result_3' name='game_result' value='3'/><label for='game_result_3'>홀</label>"
						+"	            </div>"
						+"	            <div class='col-xs-2 col-sm-2 col-md-2'>"
						+"	            	<input type='radio' id='game_result_4' name='game_result' value='4'/><label for='game_result_4'>세</label>"
						+"	            </div>"
						+"	        </div>"
						+"	    </div>"
...
function updaterecodeform(check,num) {
	...
	$.ajax({
		...
		success: function(data){
			...
			for(var m=1; m<13 ; m++){
				if(date_data[1] == m){
					selectbox2 += "<option value='"+m+"' selected>"+m+"</option>"; 
				}else{
					selectbox2 += "<option value='"+m+"'>"+m+"</option>"; 
				}
			}
			if(date_data[1]==1 || date_data[1]==3 || date_data[1]==5 || date_data[1]==7 || date_data[1]==8 || date_data[1]==10 || date_data[1]==12){
				day = 31;
			}else if(date_data[1]==2){
				if(date_data[0]%4 == 0){
					day = 29;
				}else{
					day = 28;
			...
			for(var d=1;d<=30;d++){
				if(date_data[2] == d){
					selectbox3 += "<option value='"+d+"' selected>"+d+"</option>";
				}else{
					selectbox3 += "<option value='"+d+"'>"+d+"</option>"; 
				}
			}
			if(select_data[1] == 1){
				kind_1 = "checked";
			...
			if(check==2){
				append_content = "<div class='inputrecodeform' >"
								...
								+"	</div>"
								+"	<hr>"
								+"	<form action='TestRecordUpdateProc.php' method='post'>"
								+"		<div class='form-group' id='divLeagueYear'>"
								+"	        <div class='col-xs-3 control-label'><label for='inputLeagueYear'>날짜<span style='color:red'> *</span></label></div>"
								+"	        <div class='col-xs-3 input_record_form'>"
								+"	            <select class='form-control' name='game_y' id='game_y'>"
								...
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <div class='col-xs-6 col-sm-6 col-md-6'>"
								+"	            	<input type='radio' id='kinds_1' name='kind' value='1' "+kind_1+" onclick='check_kind(1)'/><label for='kinds_1'>연습경기</label>"
								+"	            </div>"
								+"	            <div class='col-xs-6 col-sm-6 col-md-6'>"
								+"	            	<input type='radio' id='kinds_2' name='kind' value='2' "+kind_2+" onclick='check_kind(2)'/><label for='kinds_2'>용병경기</label>"
								+"	            </div>"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divTeamname'>"
								+"	        <div class='col-xs-3 control-label'><label for='inputTeamname'>팀이름</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='team_name' id='team_name' maxlength='20' value="+select_data[2]+" "+disabled_t+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divPlayBatter'>"
								+"	        <div class='col-xs-3 control-label'><label for='inputPlayBatter'>타석</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='at_play' id='' maxlength='10' value="+select_data[3]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divPlayBat'>"
								+"	        <div class='col-xs-3 control-label'><label for='inputPlayBat'>타수</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='at_bat' id='' maxlength='10' value="+select_data[4]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divPlayHit'>"
								+"	        <div class='col-xs-3 control-label'><label for='inputPlayHit'>안타</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='hit' id='' maxlength='10'  placeholder='1루타 + 2루타 + 3루타 + 홈런' value="+select_data[5]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divTwobase'>"
								+"	        <div class='col-xs-3 control-label'><label for='inputTwobase'>2루타</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='hit2' id='' maxlength='10' value="+select_data[6]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divThreebase'>"
								+"	        <div class='col-xs-3 control-label'><label for='inputThreebase'>3루타</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='hit3' id='' maxlength='10' value="+select_data[7]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divHomerun'>"
								+"	        <div class='col-xs-3 control-label'><label for='inputHomerun'>홈런</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='hr' id='' maxlength='10' value="+select_data[8]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divFourball'>"
								+"	        <div class='col-xs-3 control-label'><label for='inputFourball'>볼넷</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='bb' id='' maxlength='10' value="+select_data[9]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divDeadball'>"
								+"	        <div class='col-xs-3 control-label'><label for='inputDeadball'>사사구</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='hbp' id='' maxlength='10' value="+select_data[10]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divPlaypoint'>"
								+"	        <div class='col-xs-3 control-label'><label for='inputPlaypoint'>타점</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='rbi' id='' maxlength='10' value="+select_data[11]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divPlayrbi'>"
								+"	        <div class='col-xs-3 control-label'><label for='inputPlayrbi'>득점</label></div>"
								...
								+"	        <div class='col-xs-3 control-label'><label for='inputPlayrbi'>도루</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='sb' id='' maxlength='10' value="+select_data[13]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divThreeout'>"
								+"	        <div class='col-xs-3 control-label'><label for='inputThreeout'>삼진</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='so' id='' maxlength='10' value="+select_data[14]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divSacrifice'>"
								+"	        <div class='col-xs-3 control-label'><label for='inputSacrifice'>희생타</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='sf' id='' maxlength='10' value="+select_data[15]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div>"
				...
				if(select_data[3] == 1){
					game_result_checked_0 = "";
					game_result_checked_1 = "checked";
					game_result_checked_2 = "";
					game_result_checked_3 = "";
					game_result_checked_4 = "";
				}else if(select_data[3] == 2){
					game_result_checked_0 = "";
					game_result_checked_1 = "";
					game_result_checked_2 = "checked";
					game_result_checked_3 = "";
					game_result_checked_4 = "";
				}else if(select_data[3] == 3){
					game_result_checked_0 = "";
					game_result_checked_1 = "";
					game_result_checked_2 = "";
					game_result_checked_3 = "checked";
					game_result_checked_4 = "";
				}else if(select_data[3] == 4){
					game_result_checked_0 = "";
					game_result_checked_1 = "";
					game_result_checked_2 = "";
					game_result_checked_3 = "";
					game_result_checked_4 = "checked";
				}else{
					game_result_checked_0 = "checked";
					game_result_checked_1 = "";
					game_result_checked_2 = "";
					game_result_checked_3 = "";
					game_result_checked_4 = "";
				...
				append_content = "<div class='inputrecodeform' >"
								...
								+"	            <div class='col-xs-1 col-sm-1 col-md-1'></div>"
								+"	            <div class='col-xs-2 col-sm-2 col-md-2'>"
								+"	            	<input type='radio' id='game_result_0' name='game_result' "+game_result_checked_0+" value='0' checked/><label for='game_result_0'>-</label>"
								+"	            </div>"
								+"	            <div class='col-xs-2 col-sm-2 col-md-2'>"
								+"	            	<input type='radio' id='game_result_1' name='game_result' "+game_result_checked_1+" value='1'/><label for='game_result_1'>승</label>"
								+"	            </div>"
								+"	            <div class='col-xs-2 col-sm-2 col-md-2'>"
								+"	            	<input type='radio' id='game_result_2' name='game_result' "+game_result_checked_2+" value='2'/><label for='game_result_2'>패</label>"
								+"	            </div>"
								+"	            <div class='col-xs-2 col-sm-2 col-md-2'>"
								+"	            	<input type='radio' id='game_result_3' name='game_result' "+game_result_checked_3+" value='3'/><label for='game_result_3'>홀</label>"
								+"	            </div>"
								+"	            <div class='col-xs-2 col-sm-2 col-md-2'>"
								+"	            	<input type='radio' id='game_result_4' name='game_result' "+game_result_checked_4+" value='4'/><label for='game_result_4'>세</label>"
								+"	            </div>"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divInning'>"
								+"	        <div class='col-xs-3 control-label'><label for='inputInning'>이닝</label></div>"
								+"	        <div class='col-xs-6 input_record_form'>"
								+"	            <input type='text' class='form-control' name='h_inning' id='' maxlength='10' value="+select_data[3]+">"
								+"	        </div>"
								+"	        <div class='col-xs-3 input_record_form'>"
								+"	            <select class='form-control' name='f_inning'>"
								...
								+"	        <div class='col-xs-3 control-label'><label for='inputPitch'>투구수</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='pitcher_count' id='' maxlength='10' value="+select_data[5]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divErpoint' >"
								+"			<div class='col-xs-3 control-label'><label for='divErpoint'>자책점</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='er' id='' maxlength='10' value="+select_data[6]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divLosepoint' >"
								+"			<div class='col-xs-3 control-label'><label for='inputLosepoint'>실점</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='rs' id='' maxlength='10' value="+select_data[7]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divHits'>"
								+"	        <div class='col-xs-3 control-label'><label for='inputHits'>피안타</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='hit' id='' maxlength='10' value="+select_data[8]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divHomeruns'>"
								+"	        <div class='col-xs-3 control-label'><label for='inputHomeruns'>피홈런</label></div>"
								...
								+"	        <div class='col-xs-3 control-label'><label for='inputThreeouts'>탈삼진</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='so' id='' maxlength='10' value="+select_data[10]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divFourballs'>"
								+"	        <div class='col-xs-3 control-label'><label for='inputFourballs'>볼넷</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='bb' id='' maxlength='10' value="+select_data[11]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div class='form-group' id='divDeadballs' >"
								+"			<div class='col-xs-3 control-label'><label for='inputDeadballs'>사사구</label></div>"
								+"	        <div class='col-xs-9 input_record_form'>"
								+"	            <input type='text' class='form-control' name='hbp' id='' maxlength='10' value="+select_data[12]+">"
								+"	        </div>"
								+"	    </div>"
								+"	    <div>"
								+"	    	<input type='hidden' value='"+num+"' name='no'>"	
...
function selectmonth(obj) {		//생년월일 - 월 선택 후 일
	console.log("obj : "+obj);
	$("select#game_d option").remove();
	years = $("#game_y option:selected").val();
	var day;
	...
	for(var i=1;i<=day;i++){
		$("#game_d").append("<option value='"+i+"'>"+i+"</option>");
	}
}
function check_kind(kind){
	if(kind == 1){
		...
		document.getElementById("team_name").value = "용병";
