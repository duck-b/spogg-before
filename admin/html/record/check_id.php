...
<?php
    ...
    if(!$id){
		...
		if ($num_record){ 
			$contents = "아이디가 존재합니다.<br>다른 아이디를 사용하세요."; 
		}else{ 
...
?>
	...
	function sendTxt(){
		...
		window.opener.document.user_form.checkedid.value = "중복확인 완료";
		window.opener.document.user_form.checkedid.disabled = true;
		window.opener.document.user_form.signup_button.disabled = false;
		self.close();
	}
	function cancle(){
