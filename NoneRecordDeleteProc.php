...
if($row){
	if($_GET['pos'] == '1'){
		mysqli_query($conn,"DELETE FROM none_record_hit WHERE player='$player' AND no='$no'");
	}else if($_GET['pos'] == '2'){
		mysqli_query($conn,"DELETE FROM none_record_pit WHERE player='$player' AND no='$no'");
	}
	mysqli_close($conn);
